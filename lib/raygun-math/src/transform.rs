use super::{translation_matrix, Matrix};
...
impl Transform {
    ...
    pub fn for_rotation(x: Angle<Radians>, y: Angle<Radians>, z: Angle<Radians>) -> Transform {
    ...
    pub fn rotate(&self, x: Angle<Radians>, y: Angle<Radians>, z: Angle<Radians>) -> Transform {
...
impl Default for Transform {
    fn default() -> Transform {
        Transform {
            matrix: super::IDENTITY,
            inverse: super::IDENTITY,
        }
    }
}
...
mod test {
    ...
    fn rotation() {
        ...
        for &((rx, ry, rz), expected) in test_cases.iter() {
            let t = Transform::default().rotate(
                degrees(rx).radians(),
                degrees(ry).radians(),
                degrees(rz).radians(),
            );
